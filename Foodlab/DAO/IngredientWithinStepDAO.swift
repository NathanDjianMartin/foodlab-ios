...
struct IngredientWithinStepDAO {
    ...
    static func getAllIngredientsWithinStep(id: Int) async -> Result<[Ingredient: Double], Error> {
        ...
        do {
            let decoded : [IngredientWithinStepDTO] = try await URLSession.shared.get(from: stringUrl + "recipe-execution/all-ingredients-within-a-step-in-simple-step/\(id)")
    ...
    static func getIngredientsDicoFromIngredientDTOList(ingredientsDTO: [IngredientWithinStepDTO]) async -> Result<[Ingredient: Double], Error> {
        var ingredientsWithinStep: [Ingredient: Double] = [:]
        for ingredientWithinStepDTO in ingredientsDTO {
            
            // on recupere l'ingredient
            switch await IngredientDAO.getIngredientById(id: ingredientWithinStepDTO.ingredientId){
    ...
    static func addIngredientsInSimpleStep(stepId: Int, ingredients: [Ingredient:Double]) async -> Result<Bool, Error> {
        ...
        do {
            for (ingredient,quantity) in ingredients {
                ...
                let ingredientDTOresult : IngredientWithinStepDTO = try await URLSession.shared.create(from: stringUrl+"ingredient-within-step", object: ingredientWithinStepDTO)
            }
